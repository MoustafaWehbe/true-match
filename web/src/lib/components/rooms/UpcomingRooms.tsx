...
import { getLiveRooms } from '~/lib/state/room/roomSlice';

function UpcomingRooms() {
  const bg = useColorModeValue('gray.50', 'gray.800');
  const textColor = useColorModeValue('gray.800', 'whiteAlpha.900');
  const dispatch = useDispatch<AppDispatch>();
  const { liveRooms, getLiveRoomsLoading } = useSelector(
    (state: RootState) => state.room
  );

  useEffect(() => {
    if (!getLiveRoomsLoading) {
      dispatch(
        getLiveRooms({
          PageNumber: 1,
          PageSize: 10,
  ...
  return (
    <Box bg={bg} color={textColor} px={8} py={4} borderRadius="lg">
      ...
      <SimpleGrid columns={{ base: 1, sm: 2, md: 3 }} spacing={8} mt={8}>
        {liveRooms?.data?.map((room) => <RoomCard key={room.id} room={room} />)}
      </SimpleGrid>
    </Box>
  );
}
